...
$stats = getIndexedPageStat($profileId, $startDate, $endDate, $interval);

?>
<chart caption='Indexed Pages' lineThickness='4' showValues='0' anchorRadius='4' anchorBgAlpha='50' showAlternateVGridColor='1' animation='1' scrollToEnd='1' showLegend='1' chartTopMargin='5' chartBottomMargin='10' chartLeftMargin='5' chartRightMargin='10' captionPadding='5' palette='3' setAdaptiveYMin='1' baseFontSize='9' baseFont='Lucida Grande, Arial' valuePadding='5' connectNullData='1'>
<categories >
<? for ($i = 0; $i < sizeof($stats); $i++) { ?>
<category label='<?=$stats[$i]['period']?>' />
<? } ?>
</categories>
<dataset seriesName='Bing' color='6666FF'>
<? for ($i = 0; $i < sizeof($stats); $i++) { ?>
<set value='<?=emptyIfZero(round($stats[$i]['bing']))?>' />
<? } ?>
</dataset>
<dataset seriesName='Google'>
<? for ($i = 0; $i < sizeof($stats); $i++) { ?>
<set value='<?=emptyIfZero(round($stats[$i]['google']))?>' />
<? } ?>
</dataset>
<dataset seriesName='Yahoo'>
<? for ($i = 0; $i < sizeof($stats); $i++) { ?>
<set value='<?=emptyIfZero(round($stats[$i]['yahoo']))?>' />
<? } ?>
</dataset>
