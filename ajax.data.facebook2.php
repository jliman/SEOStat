...
<? for ($i = 0; $i < sizeof($stats); $i++) { ?>
<set value='<?=emptyIfZero(round($stats[$i]['unsubscribe']))?>' />
<? } ?>
</dataset>
<dataset seriesName='Fans'>
<? for ($i = 0; $i < sizeof($stats); $i++) { ?>
<set value='<?=emptyIfZero(round($stats[$i]['fans']))?>' />
<? } ?>
</dataset>
