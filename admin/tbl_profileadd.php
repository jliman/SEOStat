...
// Create page object
var tbl_profile_add = new ew_Page("tbl_profile_add");

// page properties
tbl_profile_add.PageID = "add"; // page ID
tbl_profile_add.FormID = "ftbl_profileadd"; // form ID
var EW_PAGE_ID = tbl_profile_add.PageID; // for backward compatibility
...
tbl_profile_add.ValidateForm = function(fobj) {
	...
	for (i=0; i<rowcnt; i++) {
		...
		elm = fobj.elements["x" + infix + "_fb_pageid"];
		if (elm && !ew_HasValue(elm))
			return ew_OnError(this, elm, ewLanguage.Phrase("EnterRequiredField") + " - <?php echo ew_JsEncode2($tbl_profile->fb_pageid->FldCaption()) ?>");
		elm = fobj.elements["x" + infix + "_twitter_page"];
		if (elm && !ew_HasValue(elm))
			return ew_OnError(this, elm, ewLanguage.Phrase("EnterRequiredField") + " - <?php echo ew_JsEncode2($tbl_profile->twitter_page->FldCaption()) ?>");
...
tbl_profile_add.ShowHighlightText = ewLanguage.Phrase("ShowHighlight"); 
tbl_profile_add.HideHighlightText = ewLanguage.Phrase("HideHighlight");
...
<p><span class="phpmaker"><?php echo $Language->Phrase("Add") ?>&nbsp;<?php echo $Language->Phrase("TblTypeTABLE") ?><?php echo $tbl_profile->TableCaption() ?><br><br>
...
?>
<form name="ftbl_profileadd" id="ftbl_profileadd" action="<?php echo ew_CurrentPage() ?>" method="post" onsubmit="return tbl_profile_add.ValidateForm(this);">
<p>
...
class ctbl_profile_add {
	...
	function Page_Main() {
		...
		$tbl_profile->RowType = EW_ROWTYPE_ADD;  // Render add type
