...
// Create page object
$tbl_user_level_permissions_add = new ctbl_user_level_permissions_add();
$Page =& $tbl_user_level_permissions_add;

// Page init
$tbl_user_level_permissions_add->Page_Init();

// Page main
$tbl_user_level_permissions_add->Page_Main();
?>
...
// Create page object
var tbl_user_level_permissions_add = new ew_Page("tbl_user_level_permissions_add");

// page properties
tbl_user_level_permissions_add.PageID = "add"; // page ID
tbl_user_level_permissions_add.FormID = "ftbl_user_level_permissionsadd"; // form ID
var EW_PAGE_ID = tbl_user_level_permissions_add.PageID; // for backward compatibility

// extend page with ValidateForm function
tbl_user_level_permissions_add.ValidateForm = function(fobj) {
...
tbl_user_level_permissions_add.Form_CustomValidate =  
 function(fobj) { // DO NOT CHANGE THIS LINE!

 	// Your custom validation code here, return false if invalid. 
 	return true;
 }
tbl_user_level_permissions_add.SelectAllKey = function(elem) {
	ew_SelectAll(elem);
	ew_ClickAll(elem);
}
<?php if (EW_CLIENT_VALIDATE) { ?>
tbl_user_level_permissions_add.ValidateRequired = true; // uses JavaScript validation
<?php } else { ?>
tbl_user_level_permissions_add.ValidateRequired = false; // no JavaScript validation
<?php } ?>
...
<p><span class="phpmaker"><?php echo $Language->Phrase("Add") ?>&nbsp;<?php echo $Language->Phrase("TblTypeTABLE") ?><?php echo $tbl_user_level_permissions->TableCaption() ?><br><br>
<a href="<?php echo $tbl_user_level_permissions->getReturnUrl() ?>"><?php echo $Language->Phrase("GoBack") ?></a></span></p>
<?php
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
$tbl_user_level_permissions_add->ShowMessage();
?>
<form name="ftbl_user_level_permissionsadd" id="ftbl_user_level_permissionsadd" action="<?php echo ew_CurrentPage() ?>" method="post" onsubmit="return tbl_user_level_permissions_add.ValidateForm(this);">
<p>
...
<input type="text" name="x_userlevelid" id="x_userlevelid" title="<?php echo $tbl_user_level_permissions->userlevelid->FldTitle() ?>" size="30" value="<?php echo $tbl_user_level_permissions->userlevelid->EditValue ?>"<?php echo $tbl_user_level_permissions->userlevelid->EditAttributes() ?>>
...
<input type="text" name="x_ztablename" id="x_ztablename" title="<?php echo $tbl_user_level_permissions->ztablename->FldTitle() ?>" size="30" maxlength="80" value="<?php echo $tbl_user_level_permissions->ztablename->EditValue ?>"<?php echo $tbl_user_level_permissions->ztablename->EditAttributes() ?>>
...
<?php
$tbl_user_level_permissions_add->Page_Terminate();
?>
<?php
...
// Page class
//
class ctbl_user_level_permissions_add {
	...
	// Page object name
	var $PageObjName = 'tbl_user_level_permissions_add';
	...
	//
	function ctbl_user_level_permissions_add() {
	...
	function Page_Main() {
		...
		if (@$_POST["a_add"] <> "") {
		   $tbl_user_level_permissions->CurrentAction = $_POST["a_add"]; // Get form action
		  $this->LoadFormValues(); // Load form values

			// Validate form
			if (!$this->ValidateForm()) {
				$tbl_user_level_permissions->CurrentAction = "I"; // Form error, reset action
				$this->setMessage($gsFormError);
			}
		} else { // Not post back
		  if ($bCopy) {
		    $tbl_user_level_permissions->CurrentAction = "C"; // Copy record
		  } else {
		    $tbl_user_level_permissions->CurrentAction = "I"; // Display blank record
		...
		switch ($tbl_user_level_permissions->CurrentAction) {
		  ...
		  case "A": // ' Add new record
				$tbl_user_level_permissions->SendEmail = TRUE; // Send email on add success
		    if ($this->AddRow()) { // Add successful
		      $this->setMessage($Language->Phrase("AddSuccess")); // Set up success message
					...
					if (ew_GetPageName($sReturnUrl) == "tbl_user_level_permissionsview.php")
						$sReturnUrl = $tbl_user_level_permissions->ViewUrl(); // View paging, return to view page with keyurl directly
		...
		$tbl_user_level_permissions->RowType = EW_ROWTYPE_ADD;  // Render add type
	...
	function RenderRow() {
		...
		} elseif ($tbl_user_level_permissions->RowType == EW_ROWTYPE_ADD) { // Add row

			// userlevelid
			$tbl_user_level_permissions->userlevelid->EditCustomAttributes = "";
			$tbl_user_level_permissions->userlevelid->EditValue = ew_HtmlEncode($tbl_user_level_permissions->userlevelid->CurrentValue);

			// tablename
			$tbl_user_level_permissions->ztablename->EditCustomAttributes = "";
			$tbl_user_level_permissions->ztablename->EditValue = ew_HtmlEncode($tbl_user_level_permissions->ztablename->CurrentValue);
	...
	function AddRow() {
		...
		$sFilter = $tbl_user_level_permissions->KeyFilter();
		if ($bCheckKey) {
			$rsChk = $tbl_user_level_permissions->LoadRs($sFilter);
			if ($rsChk && !$rsChk->EOF) {
		...
		// userlevelid
		$tbl_user_level_permissions->userlevelid->SetDbValueDef($rsnew, $tbl_user_level_permissions->userlevelid->CurrentValue, 0, FALSE);

		// tablename
		$tbl_user_level_permissions->ztablename->SetDbValueDef($rsnew, $tbl_user_level_permissions->ztablename->CurrentValue, "", FALSE);

		// permission
		$tbl_user_level_permissions->permission->SetDbValueDef($rsnew, $tbl_user_level_permissions->permission->CurrentValue, 0, FALSE);

		// Call Row Inserting event
		$bInsertRow = $tbl_user_level_permissions->Row_Inserting($rsnew);
		if ($bInsertRow) {
			$conn->raiseErrorFn = 'ew_ErrorFn';
			$AddRow = $conn->Execute($tbl_user_level_permissions->InsertSQL($rsnew));
			$conn->raiseErrorFn = '';
		} else {
		...
		if ($AddRow) {
			...
			$tbl_user_level_permissions->Row_Inserted($rsnew);
		}
		return $AddRow;
	}
