...
// Create page object
$tbl_ga_stat_edit = new ctbl_ga_stat_edit();
$Page =& $tbl_ga_stat_edit;

// Page init
$tbl_ga_stat_edit->Page_Init();

// Page main
$tbl_ga_stat_edit->Page_Main();
?>
...
// Create page object
var tbl_ga_stat_edit = new ew_Page("tbl_ga_stat_edit");

// page properties
tbl_ga_stat_edit.PageID = "edit"; // page ID
tbl_ga_stat_edit.FormID = "ftbl_ga_statedit"; // form ID
var EW_PAGE_ID = tbl_ga_stat_edit.PageID; // for backward compatibility

// extend page with ValidateForm function
tbl_ga_stat_edit.ValidateForm = function(fobj) {
...
tbl_ga_stat_edit.Form_CustomValidate =  
 function(fobj) { // DO NOT CHANGE THIS LINE!

 	// Your custom validation code here, return false if invalid. 
 	return true;
 }
tbl_ga_stat_edit.SelectAllKey = function(elem) {
	ew_SelectAll(elem);
	ew_ClickAll(elem);
}
<?php if (EW_CLIENT_VALIDATE) { ?>
tbl_ga_stat_edit.ValidateRequired = true; // uses JavaScript validation
<?php } else { ?>
tbl_ga_stat_edit.ValidateRequired = false; // no JavaScript validation
<?php } ?>

// search highlight properties
tbl_ga_stat_edit.ShowHighlightText = ewLanguage.Phrase("ShowHighlight"); 
tbl_ga_stat_edit.HideHighlightText = ewLanguage.Phrase("HideHighlight");
...
<p><span class="phpmaker"><?php echo $Language->Phrase("Edit") ?>&nbsp;<?php echo $Language->Phrase("TblTypeTABLE") ?><?php echo $tbl_ga_stat->TableCaption() ?><br><br>
<a href="<?php echo $tbl_ga_stat->getReturnUrl() ?>"><?php echo $Language->Phrase("GoBack") ?></a></span></p>
<?php
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
$tbl_ga_stat_edit->ShowMessage();
?>
<form name="ftbl_ga_statedit" id="ftbl_ga_statedit" action="<?php echo ew_CurrentPage() ?>" method="post" onsubmit="return tbl_ga_stat_edit.ValidateForm(this);">
<p>
<input type="hidden" name="a_table" id="a_table" value="tbl_ga_stat">
...
<div<?php echo $tbl_ga_stat->id_profile->ViewAttributes() ?>><?php echo $tbl_ga_stat->id_profile->EditValue ?></div><input type="hidden" name="x_id_profile" id="x_id_profile" value="<?php echo ew_HtmlEncode($tbl_ga_stat->id_profile->CurrentValue) ?>">
...
<div<?php echo $tbl_ga_stat->stat_date->ViewAttributes() ?>><?php echo $tbl_ga_stat->stat_date->EditValue ?></div><input type="hidden" name="x_stat_date" id="x_stat_date" value="<?php echo ew_HtmlEncode($tbl_ga_stat->stat_date->CurrentValue) ?>">
...
<div<?php echo $tbl_ga_stat->hour->ViewAttributes() ?>><?php echo $tbl_ga_stat->hour->EditValue ?></div><input type="hidden" name="x_hour" id="x_hour" value="<?php echo ew_HtmlEncode($tbl_ga_stat->hour->CurrentValue) ?>">
...
<?php
$tbl_ga_stat_edit->Page_Terminate();
?>
<?php
...
class ctbl_ga_stat_edit {
	...
	// Page object name
	var $PageObjName = 'tbl_ga_stat_edit';
	...
	function Page_Main() {
		...
		if (@$_POST["a_edit"] <> "") {
			$tbl_ga_stat->CurrentAction = $_POST["a_edit"]; // Get action code
			$this->LoadFormValues(); // Get form values

			// Validate form
			if (!$this->ValidateForm()) {
				$tbl_ga_stat->CurrentAction = ""; // Form error, reset action
				$this->setMessage($gsFormError);
				$tbl_ga_stat->EventCancelled = TRUE; // Event cancelled
				$this->RestoreFormValues();
			}
		} else {
			$tbl_ga_stat->CurrentAction = "I"; // Default action is display
		}

		// Check if valid key
		if ($tbl_ga_stat->id_profile->CurrentValue == "")
			$this->Page_Terminate("tbl_ga_statlist.php"); // Invalid key, return to list
			...
			Case "U": // Update
				...
				if ($this->EditRow()) { // Update record based on key
					...
					if (ew_GetPageName($sReturnUrl) == "tbl_ga_statview.php")
						$sReturnUrl = $tbl_ga_stat->ViewUrl(); // View paging, return to View page directly
		...
		// Render the record
		$tbl_ga_stat->RowType = EW_ROWTYPE_EDIT; // Render as Edit
		$this->RenderRow();
	}
