...
// Create page object
$tbl_backlink_stat_delete = new ctbl_backlink_stat_delete();
$Page =& $tbl_backlink_stat_delete;

// Page init
$tbl_backlink_stat_delete->Page_Init();

// Page main
$tbl_backlink_stat_delete->Page_Main();
?>
...
// Create page object
var tbl_backlink_stat_delete = new ew_Page("tbl_backlink_stat_delete");

// page properties
tbl_backlink_stat_delete.PageID = "delete"; // page ID
tbl_backlink_stat_delete.FormID = "ftbl_backlink_statdelete"; // form ID
var EW_PAGE_ID = tbl_backlink_stat_delete.PageID; // for backward compatibility

// extend page with Form_CustomValidate function
tbl_backlink_stat_delete.Form_CustomValidate =  
 function(fobj) { // DO NOT CHANGE THIS LINE!

 	// Your custom validation code here, return false if invalid. 
 	return true;
 }
tbl_backlink_stat_delete.SelectAllKey = function(elem) {
	ew_SelectAll(elem);
	ew_ClickAll(elem);
}
<?php if (EW_CLIENT_VALIDATE) { ?>
tbl_backlink_stat_delete.ValidateRequired = true; // uses JavaScript validation
<?php } else { ?>
tbl_backlink_stat_delete.ValidateRequired = false; // no JavaScript validation
<?php } ?>

// search highlight properties
tbl_backlink_stat_delete.ShowHighlightText = ewLanguage.Phrase("ShowHighlight"); 
tbl_backlink_stat_delete.HideHighlightText = ewLanguage.Phrase("HideHighlight");
...
if ($rs = $tbl_backlink_stat_delete->LoadRecordset())
	$tbl_backlink_stat_deletelTotalRecs = $rs->RecordCount(); // Get record count
if ($tbl_backlink_stat_deletelTotalRecs <= 0) { // No record found, exit
	if ($rs)
		$rs->Close();
	$tbl_backlink_stat_delete->Page_Terminate("tbl_backlink_statlist.php"); // Return to list
}
?>
<p><span class="phpmaker"><?php echo $Language->Phrase("Delete") ?>&nbsp;<?php echo $Language->Phrase("TblTypeTABLE") ?><?php echo $tbl_backlink_stat->TableCaption() ?><br><br>
<a href="<?php echo $tbl_backlink_stat->getReturnUrl() ?>"><?php echo $Language->Phrase("GoBack") ?></a></span></p>
<?php
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
$tbl_backlink_stat_delete->ShowMessage();
?>
<form action="<?php echo ew_CurrentPage() ?>" method="post">
<p>
<input type="hidden" name="t" id="t" value="tbl_backlink_stat">
<input type="hidden" name="a_delete" id="a_delete" value="D">
<?php foreach ($tbl_backlink_stat_delete->arRecKeys as $key) { ?>
...
<?php echo $tbl_backlink_stat->TableCustomInnerHtml ?>
	...
	<tr class="ewTableHeader">
		<td valign="top"><?php echo $tbl_backlink_stat->id_profile->FldCaption() ?></td>
		<td valign="top"><?php echo $tbl_backlink_stat->stat_date->FldCaption() ?></td>
		<td valign="top"><?php echo $tbl_backlink_stat->year->FldCaption() ?></td>
		<td valign="top"><?php echo $tbl_backlink_stat->month->FldCaption() ?></td>
		<td valign="top"><?php echo $tbl_backlink_stat->week->FldCaption() ?></td>
		<td valign="top"><?php echo $tbl_backlink_stat->backlink1->FldCaption() ?></td>
		<td valign="top"><?php echo $tbl_backlink_stat->backlink2->FldCaption() ?></td>
		<td valign="top"><?php echo $tbl_backlink_stat->backlink3->FldCaption() ?></td>
	</tr>
	</thead>
	<tbody>
<?php
$tbl_backlink_stat_delete->lRecCnt = 0;
$i = 0;
while (!$rs->EOF) {
	$tbl_backlink_stat_delete->lRecCnt++;

	// Set row properties
	$tbl_backlink_stat->CssClass = "";
	$tbl_backlink_stat->CssStyle = "";
	$tbl_backlink_stat->RowAttrs = array();
	$tbl_backlink_stat->RowType = EW_ROWTYPE_VIEW; // View

	// Get the field contents
	$tbl_backlink_stat_delete->LoadRowValues($rs);

	// Render row
	$tbl_backlink_stat_delete->RenderRow();
?>
	<tr<?php echo $tbl_backlink_stat->RowAttributes() ?>>
		<td<?php echo $tbl_backlink_stat->id_profile->CellAttributes() ?>>
<div<?php echo $tbl_backlink_stat->id_profile->ViewAttributes() ?>><?php echo $tbl_backlink_stat->id_profile->ListViewValue() ?></div></td>
		<td<?php echo $tbl_backlink_stat->stat_date->CellAttributes() ?>>
<div<?php echo $tbl_backlink_stat->stat_date->ViewAttributes() ?>><?php echo $tbl_backlink_stat->stat_date->ListViewValue() ?></div></td>
		<td<?php echo $tbl_backlink_stat->year->CellAttributes() ?>>
<div<?php echo $tbl_backlink_stat->year->ViewAttributes() ?>><?php echo $tbl_backlink_stat->year->ListViewValue() ?></div></td>
		<td<?php echo $tbl_backlink_stat->month->CellAttributes() ?>>
<div<?php echo $tbl_backlink_stat->month->ViewAttributes() ?>><?php echo $tbl_backlink_stat->month->ListViewValue() ?></div></td>
		<td<?php echo $tbl_backlink_stat->week->CellAttributes() ?>>
<div<?php echo $tbl_backlink_stat->week->ViewAttributes() ?>><?php echo $tbl_backlink_stat->week->ListViewValue() ?></div></td>
		<td<?php echo $tbl_backlink_stat->backlink1->CellAttributes() ?>>
<div<?php echo $tbl_backlink_stat->backlink1->ViewAttributes() ?>><?php echo $tbl_backlink_stat->backlink1->ListViewValue() ?></div></td>
		<td<?php echo $tbl_backlink_stat->backlink2->CellAttributes() ?>>
<div<?php echo $tbl_backlink_stat->backlink2->ViewAttributes() ?>><?php echo $tbl_backlink_stat->backlink2->ListViewValue() ?></div></td>
		<td<?php echo $tbl_backlink_stat->backlink3->CellAttributes() ?>>
<div<?php echo $tbl_backlink_stat->backlink3->ViewAttributes() ?>><?php echo $tbl_backlink_stat->backlink3->ListViewValue() ?></div></td>
	</tr>
<?php
	$rs->MoveNext();
}
...
<?php
$tbl_backlink_stat_delete->Page_Terminate();
?>
<?php
...
// Page class
//
class ctbl_backlink_stat_delete {
	...
	// Page object name
	var $PageObjName = 'tbl_backlink_stat_delete';
	...
	//
	function ctbl_backlink_stat_delete() {
	...
	function Page_Main() {
		...
		if (@$_GET["id_profile"] <> "") {
			...
			if (!is_numeric($tbl_backlink_stat->id_profile->QueryStringValue))
				$this->Page_Terminate("tbl_backlink_statlist.php"); // Prevent SQL injection, exit
			$sKey .= $tbl_backlink_stat->id_profile->QueryStringValue;
		} else {
			$bSingleDelete = FALSE;
		}
		if (@$_GET["stat_date"] <> "") {
			...
			$sKey .= $tbl_backlink_stat->stat_date->QueryStringValue;
		} else {
			$bSingleDelete = FALSE;
		}
		if ($bSingleDelete) {
		...
		if ($nKeySelected <= 0)
			$this->Page_Terminate("tbl_backlink_statlist.php"); // No key specified, return to list
		...
		foreach ($this->arRecKeys as $sKey) {
			...
			if (count($arKeyFlds) <> 2)
				$this->Page_Terminate($tbl_backlink_stat->getReturnUrl()); // Invalid key, exit

			// Set up key field
			$sKeyFld = $arKeyFlds[0];
			if (!is_numeric($sKeyFld))
				$this->Page_Terminate("tbl_backlink_statlist.php"); // Prevent SQL injection, return to list
		...
		// SQL constructor in tbl_backlink_stat class, tbl_backlink_statinfo.php

		$tbl_backlink_stat->CurrentFilter = $sFilter;

		// Get action
		if (@$_POST["a_delete"] <> "") {
			$tbl_backlink_stat->CurrentAction = $_POST["a_delete"];
		} else {
			$tbl_backlink_stat->CurrentAction = "D"; // Delete record directly
		}
		switch ($tbl_backlink_stat->CurrentAction) {
			case "D": // Delete
				$tbl_backlink_stat->SendEmail = TRUE; // Send email on delete success
				if ($this->DeleteRows()) { // delete rows
					$this->setMessage($Language->Phrase("DeleteSuccess")); // Set up success message
					$this->Page_Terminate($tbl_backlink_stat->getReturnUrl()); // Return to caller
				}
		}
	}
	...
	function DeleteRows() {
		...
		$DeleteRows = TRUE;
		$sWrkFilter = $tbl_backlink_stat->CurrentFilter;

		// Set up filter (SQL WHERE clause) and get return SQL
		// SQL constructor in tbl_backlink_stat class, tbl_backlink_statinfo.php

		$tbl_backlink_stat->CurrentFilter = $sWrkFilter;
		...
		if ($DeleteRows) {
			foreach ($rsold as $row) {
				$DeleteRows = $tbl_backlink_stat->Row_Deleting($row);
				if (!$DeleteRows) break;
			}
		}
		if ($DeleteRows) {
			...
			foreach ($rsold as $row) {
				...
				$conn->raiseErrorFn = 'ew_ErrorFn';
				$DeleteRows = $conn->Execute($tbl_backlink_stat->DeleteSQL($row)); // Delete
				$conn->raiseErrorFn = '';
				if ($DeleteRows === FALSE)
					break;
				if ($sKey <> "") $sKey .= ", ";
		...
		if ($DeleteRows) {
			foreach ($rsold as $row) {
				$tbl_backlink_stat->Row_Deleted($row);
			}	
		}
		return $DeleteRows;
	}
