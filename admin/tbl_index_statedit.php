...
// Create page object
$tbl_index_stat_edit = new ctbl_index_stat_edit();
$Page =& $tbl_index_stat_edit;

// Page init
$tbl_index_stat_edit->Page_Init();

// Page main
$tbl_index_stat_edit->Page_Main();
?>
...
// Create page object
var tbl_index_stat_edit = new ew_Page("tbl_index_stat_edit");

// page properties
tbl_index_stat_edit.PageID = "edit"; // page ID
tbl_index_stat_edit.FormID = "ftbl_index_statedit"; // form ID
var EW_PAGE_ID = tbl_index_stat_edit.PageID; // for backward compatibility

// extend page with ValidateForm function
tbl_index_stat_edit.ValidateForm = function(fobj) {
...
tbl_index_stat_edit.Form_CustomValidate =  
 function(fobj) { // DO NOT CHANGE THIS LINE!

 	// Your custom validation code here, return false if invalid. 
 	return true;
 }
tbl_index_stat_edit.SelectAllKey = function(elem) {
	ew_SelectAll(elem);
	ew_ClickAll(elem);
}
<?php if (EW_CLIENT_VALIDATE) { ?>
tbl_index_stat_edit.ValidateRequired = true; // uses JavaScript validation
<?php } else { ?>
tbl_index_stat_edit.ValidateRequired = false; // no JavaScript validation
<?php } ?>

// search highlight properties
tbl_index_stat_edit.ShowHighlightText = ewLanguage.Phrase("ShowHighlight"); 
tbl_index_stat_edit.HideHighlightText = ewLanguage.Phrase("HideHighlight");
...
<p><span class="phpmaker"><?php echo $Language->Phrase("Edit") ?>&nbsp;<?php echo $Language->Phrase("TblTypeTABLE") ?><?php echo $tbl_index_stat->TableCaption() ?><br><br>
<a href="<?php echo $tbl_index_stat->getReturnUrl() ?>"><?php echo $Language->Phrase("GoBack") ?></a></span></p>
<?php
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
$tbl_index_stat_edit->ShowMessage();
?>
<form name="ftbl_index_statedit" id="ftbl_index_statedit" action="<?php echo ew_CurrentPage() ?>" method="post" onsubmit="return tbl_index_stat_edit.ValidateForm(this);">
<p>
<input type="hidden" name="a_table" id="a_table" value="tbl_index_stat">
...
<div<?php echo $tbl_index_stat->id_profile->ViewAttributes() ?>><?php echo $tbl_index_stat->id_profile->EditValue ?></div><input type="hidden" name="x_id_profile" id="x_id_profile" value="<?php echo ew_HtmlEncode($tbl_index_stat->id_profile->CurrentValue) ?>">
...
<div<?php echo $tbl_index_stat->stat_date->ViewAttributes() ?>><?php echo $tbl_index_stat->stat_date->EditValue ?></div><input type="hidden" name="x_stat_date" id="x_stat_date" value="<?php echo ew_HtmlEncode($tbl_index_stat->stat_date->CurrentValue) ?>">
...
<?php
$tbl_index_stat_edit->Page_Terminate();
?>
<?php
...
class ctbl_index_stat_edit {
	...
	// Page object name
	var $PageObjName = 'tbl_index_stat_edit';
	...
	function Page_Main() {
		...
		if (@$_POST["a_edit"] <> "") {
			$tbl_index_stat->CurrentAction = $_POST["a_edit"]; // Get action code
			$this->LoadFormValues(); // Get form values

			// Validate form
			if (!$this->ValidateForm()) {
				$tbl_index_stat->CurrentAction = ""; // Form error, reset action
				$this->setMessage($gsFormError);
				$tbl_index_stat->EventCancelled = TRUE; // Event cancelled
				$this->RestoreFormValues();
			}
		} else {
			$tbl_index_stat->CurrentAction = "I"; // Default action is display
		}

		// Check if valid key
		if ($tbl_index_stat->id_profile->CurrentValue == "")
			$this->Page_Terminate("tbl_index_statlist.php"); // Invalid key, return to list
			...
			Case "U": // Update
				...
				if ($this->EditRow()) { // Update record based on key
					...
					if (ew_GetPageName($sReturnUrl) == "tbl_index_statview.php")
						$sReturnUrl = $tbl_index_stat->ViewUrl(); // View paging, return to View page directly
		...
		// Render the record
		$tbl_index_stat->RowType = EW_ROWTYPE_EDIT; // Render as Edit
		$this->RenderRow();
	}
