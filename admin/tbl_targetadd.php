...
// Create page object
$tbl_target_add = new ctbl_target_add();
$Page =& $tbl_target_add;

// Page init
$tbl_target_add->Page_Init();

// Page main
$tbl_target_add->Page_Main();
?>
...
// Create page object
var tbl_target_add = new ew_Page("tbl_target_add");

// page properties
tbl_target_add.PageID = "add"; // page ID
tbl_target_add.FormID = "ftbl_targetadd"; // form ID
var EW_PAGE_ID = tbl_target_add.PageID; // for backward compatibility

// extend page with ValidateForm function
tbl_target_add.ValidateForm = function(fobj) {
...
tbl_target_add.Form_CustomValidate =  
 function(fobj) { // DO NOT CHANGE THIS LINE!

 	// Your custom validation code here, return false if invalid. 
 	return true;
 }
tbl_target_add.SelectAllKey = function(elem) {
	ew_SelectAll(elem);
	ew_ClickAll(elem);
}
<?php if (EW_CLIENT_VALIDATE) { ?>
tbl_target_add.ValidateRequired = true; // uses JavaScript validation
<?php } else { ?>
tbl_target_add.ValidateRequired = false; // no JavaScript validation
<?php } ?>

// search highlight properties
tbl_target_add.ShowHighlightText = ewLanguage.Phrase("ShowHighlight"); 
tbl_target_add.HideHighlightText = ewLanguage.Phrase("HideHighlight");
...
<p><span class="phpmaker"><?php echo $Language->Phrase("Add") ?>&nbsp;<?php echo $Language->Phrase("TblTypeTABLE") ?><?php echo $tbl_target->TableCaption() ?><br><br>
<a href="<?php echo $tbl_target->getReturnUrl() ?>"><?php echo $Language->Phrase("GoBack") ?></a></span></p>
<?php
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
$tbl_target_add->ShowMessage();
?>
<form name="ftbl_targetadd" id="ftbl_targetadd" action="<?php echo ew_CurrentPage() ?>" method="post" onsubmit="return tbl_target_add.ValidateForm(this);">
<p>
...
class ctbl_target_add {
	...
	// Page object name
	var $PageObjName = 'tbl_target_add';
	...
	function Page_Main() {
		...
		if (@$_POST["a_add"] <> "") {
		   $tbl_target->CurrentAction = $_POST["a_add"]; // Get form action
		  $this->LoadFormValues(); // Load form values

			// Validate form
			if (!$this->ValidateForm()) {
				$tbl_target->CurrentAction = "I"; // Form error, reset action
				$this->setMessage($gsFormError);
			}
		} else { // Not post back
		  if ($bCopy) {
		    $tbl_target->CurrentAction = "C"; // Copy record
		  } else {
		    $tbl_target->CurrentAction = "I"; // Display blank record
		    ...
		    if ($this->AddRow()) { // Add successful
		      $this->setMessage($Language->Phrase("AddSuccess")); // Set up success message
					...
					if (ew_GetPageName($sReturnUrl) == "tbl_targetview.php")
						$sReturnUrl = $tbl_target->ViewUrl(); // View paging, return to view page with keyurl directly
		...
		$tbl_target->RowType = EW_ROWTYPE_ADD;  // Render add type
